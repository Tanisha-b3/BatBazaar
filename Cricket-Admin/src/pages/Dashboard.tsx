...
export default function Dashboard() {
  ...
  const fetchData = async (soft = false) => {
    ...
    try {
      ...
      if (!token) { setError('Authentication required. Please login again.'); return; }
  ...
  if (error) return (
    <div className="flex items-center justify-center h-80 p-6" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div className="bg-white border border-red-100 rounded-2xl p-8 max-w-sm w-full text-center shadow-sm">
        ...
        <button
          onClick={() => fetchData()}
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#3F51B5] text-white text-sm font-medium rounded-xl hover:bg-stone-700 transition-colors"
        >
